...
import { hasRaceDistance, lapsShort, splitTrackName } from '../utils/helpers';
...
export function LapsBadge({ laps }: LapsBadgeProps) {
  // A chip with no number reads as a rendering bug, so treat it as missing.
  if (!hasRaceDistance(laps)) return null;
